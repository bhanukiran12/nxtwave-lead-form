  ...
  try {
    ...
    if (!phoneNumber) {
      return {
        statusCode: 400,
        headers: corsHeaders,
        body: JSON.stringify({ ok: false, error: 'phoneNumber is required' })
      };
    }
    ...
    try {
      ...
      return {
        statusCode: 502,
        headers: corsHeaders,
        body: JSON.stringify({ ok: false, error: 'Draft user API failed. UUID not available.' })
      };
    }
    ...
    return {
      statusCode: 200,
      headers: corsHeaders,
      body: JSON.stringify({ ok: true, uuid })
    };
  } catch (err) {
    ...
    return {
      statusCode: 500,
      headers: corsHeaders,
      body: JSON.stringify({ ok: false, error: err.message || 'Unexpected server error' })
    };
  }
};
